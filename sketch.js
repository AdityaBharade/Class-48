...
var map;
var start;
var catsound,dogsound,batsound,germsound ,hurtsound;
var score = 0;
...
function preload(){
  ...
  bimg = loadImage("images/start.jpg");
  cimg = loadImage("images/bcat.png");
  catsound = loadSound("sounds/cats meow.wav");
...
function setup() {
  ...
  map.addImage("map", bimg);
  map.scale = 4.5
  push();
  tint(100);
  pop(); 
    bigcat=createSprite(100,100,20,20);
    bigcat.addImage('l',cimg);
    bigcat.scale=1;
...
gameState = WIN
  ...
  if(gameState===START){
    ...
    textFont("popins")
    fill("red")
    text("CAN YOU HELP BABY CAT REACH MOTHER CAT!!!",70,390)
    pop()
...
textSize(50)
   fill("red")
   text("Thank you!!! :)",600,200);
    text("You Saved Baby Cat!!! :) :)",500,250);
    pop()
   }
